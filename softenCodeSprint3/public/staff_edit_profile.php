<?php 
  ...
  if (isset($_POST['btn-submit-prof'])) {    
    ...
    $sql = "UPDATE staff SET fname='$fname', lname='$lname', addr='$addr', mobile='$mobile' WHERE email = '$email'";
    ...
    header("location: staff_edit_profile.php?alert=profileSaved");
  }
  ...
  if (isset($_POST['btn-submit-pw'])) {   
    ...
    if(!$uppercase || !$lowercase || !$number || strlen($password) < 8 || $password!=$repassword) {
      header("location: staff_edit_profile.php?alert=passwordChangedFailed");
    }else{
      if(strcmp($password,$repassword) == 0){
        $sql = "UPDATE staff SET password='$password' WHERE email = '$email'";
        $statement = $connect->prepare($sql);
        $statement->execute();
        header("location: staff_edit_profile.php?alert=passwordChanged");
      }else{
        header("location: staff_edit_profile.php?alert=passwordChangedFailed");
      }
    }
...
<body data-gr-c-s-loaded="true">
  ...
  <?php
      if($_GET['alert']=="profileSaved"){
        ...
        echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
                <h5><strong>แจ้งเตือน! </strong>เปลี่ยนหัสผ่านสำเร็จ</h5>
              </div>';
      ...
      <nav class="col-md-2 d-none d-md-block bg-light sidebar">
        ...
        <div class="sidebar-sticky">
          <ul class="nav flex-column">
            ...
            <li class="nav-item">
              <a class="nav-link active" href="#" id="setting" name="setting">
          ...
          <form method="post">
            <table>
              ...
              <tr>
                ...
                <td><input type="text" name="work" id="work" class="form-control" value="<?php echo $user['position_th']; ?>" readonly><br></td>
              </tr>
            </table>
            <br><br>
            <div id="btn-group" name="btn-group">
              <a href="<?php echo $dashboard ?>" type="button" class="btn btn-dark" value="ย้อนกลับ" >ย้อนกลับ</a>
                        ...
                        <div class="container">
                          <table>
                            <tr>
                              ...
                              <td><input class="form-control" placeholder="กรุณากรอกรหัสผ่าน" type="password" value="<?php echo $user['password']; ?>" id="psw" name="psw" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required><br></td>
                            </tr>
                            <tr>
                              <td><label >กรอกรหัสผ่านอีกครั้ง <font color=red>*</font>: </label></td>
                              <td><input type="password" name="repsw" id="repsw" class="form-control" value="<?php echo $user['password']; ?>" placeholder="กรุณากรอกรหัสผ่านอีกครั้ง" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required><br></td>
                            </tr>
                          </table>
                        </div>
